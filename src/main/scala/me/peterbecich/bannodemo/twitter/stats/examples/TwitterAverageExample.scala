...
import me.peterbecich.bannodemo.twitter.stats.TwitterAverage
...
object TwitterAverageExample {
  ...
  private lazy val makeTweetAverage: IO[TwitterAverage] =
  ...
  def watchPayloadStream(payloadStream: Stream[IO, TwitterAverage.JSON.AveragePayload]):
      Stream[IO, Unit] =
    schedulerStream.flatMap { scheduler =>
  ...
  val averageTwitter: IO[Unit] = Monad[IO].flatten({
    for {
      ...
      tweetAverage <- makeTweetAverage
      _ <- IO(println("acquired Twitter stream and average pipe"))
    } yield twitterStream
      .through(tweetAverage.averagePipe)
      .concurrently(watchPayloadStream(tweetAverage.averagePayloadStream))
  ...
  def main(args: Array[String]): Unit = {
    println("twitter average (only one) example")
